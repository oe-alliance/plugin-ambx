/*
Based on AiO Dreambox Screengrabber v0.83
*/
...
void YUVtoRGB(unsigned char *video, Bitmap* bmpLuma, Bitmap* bmpChroma)
{
	const unsigned char* luma = bmpLuma->data;
	const unsigned char* chroma = bmpChroma->data;
	int lumastride = bmpLuma->stride;
	int res = bmpLuma->height;

	int y, x, pos;
		
	int out1=0;
	int rgbstride = bmpLuma->width * 3;
	
	for (y=0; y != res; y += 2)
	{
		pos = y * lumastride;
		int t = (y>>1) * bmpChroma->stride;
		for (x=bmpLuma->width; x != 0; x-=2)
		{
			...
			Y=yuv2rgbtable_y[luma[lumastride+pos]];
